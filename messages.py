...
from util import token_required, create_response
...
def get_application_messages(current_user):
    ...
    messages = [message.data() for message in current_user.application.messages]
...
def get_application_message(current_user,message_id):
    ...
    messages = [message for message in current_user.application.messages if message.id == message_id]
    if len(messages) < 1:
...
def delete_application_message(current_user,message_id):
    ...
    current_user.application.messages = [message for message in current_user.application.messages if message.id != message_id]
    db.session.commit()
