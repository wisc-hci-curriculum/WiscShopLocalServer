...
def get_foods(current_user,meal_id):
    meal_data = Meal.query.filter_by(user_id=current_user.id,id=meal_id).first()
...
def create_food(current_user,meal_id):
    try:
        ...
        new_food = Food(meal_id=meal.id,name=name,calories=calories,protein=protein,carbohydrates=carbohydrates,fat=fat)
