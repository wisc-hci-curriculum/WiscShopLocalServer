...
def create_activity(current_user):
    ...
    new_activity = Activity(name=name, duration=duration, date=date, calories=calories, user_id=current_user.id)
...
def get_activity(current_user,activity_id):
    activity = Activity.query.filter_by(user_id=current_user.id,id=activity_id).first()
